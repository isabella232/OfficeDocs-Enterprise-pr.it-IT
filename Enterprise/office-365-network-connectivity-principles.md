...
Poiché Microsoft 365 viene eseguito sulla rete globale Microsoft, che include server front-end in tutto il mondo, spesso vi sarà un server front-end vicino alla posizione dell'utente. Fornendo l'uscita Internet locale e configurando i server DNS interni per fornire la risoluzione dei nomi locali per gli endpoint di Microsoft 365, il traffico di rete destinato a Microsoft 365 può connettersi a Microsoft 365 front end server il più vicino possibile all'utente. Nel diagramma seguente viene illustrato un esempio di topologia di rete che consente agli utenti di connettersi dalla sede principale, dalla succursale e dalle posizioni remote per seguire la route più breve verso il punto di ingresso Microsoft 365 più vicino.
...
Anche se l'uscita Regional può funzionare bene per Microsoft 365, il modello di connettività ottimale potrebbe essere quello di fornire sempre l'uscita di rete nella posizione dell'utente, indipendentemente dal fatto che si trovi sulla rete aziendale o su percorsi remoti, ad esempio case, Alberghi, coffee shop e aeroporti. Questo modello di uscita diretta locale è rappresentato nel diagramma seguente.
...
Se si utilizzano i servizi di sicurezza o di rete basati sul cloud per il traffico Microsoft 365, assicurarsi che il risultato della tornante venga valutato e che il suo impatto sulle prestazioni di Microsoft 365 sia compreso. A tale scopo, è possibile esaminare il numero e le posizioni delle posizioni dei provider di servizi tramite cui viene inoltrato il traffico in relazione al numero delle succursali e dei punti di peering della rete globale di Microsoft, la qualità della relazione di peering di rete del provider di servizi con l'ISP e Microsoft e l'impatto sulle prestazioni del backhauling nell'infrastruttura del provider di servizi.
  
A causa del numero elevato di posizioni distribuite con i punti di ingresso di Microsoft 365 e della prossimità degli utenti finali, il routing del traffico Microsoft 365 a qualsiasi rete di terze parti o provider di sicurezza può avere un impatto negativo sulle connessioni di Microsoft 365 se la rete del provider non è configurata per l'peering Microsoft 365 ottimale.
...
![Bypassare i proxy, i dispositivi di ispezione del traffico e le tecnologie di sicurezza duplicate](media/0131930d-c6cb-4ae1-bbff-fe4cf6939a23.png)
...
Nel nuovo modello gli endpoint sono suddivisi in tre categorie, **optimize**, **Allow**e **default**, fornendo un pivot basato sulla priorità in cui si concentrano gli sforzi di ottimizzazione della rete per realizzare i migliori miglioramenti delle prestazioni e restituire l'investimento. Gli endpoint sono consolidati nelle categorie sopra riportate in base alla sensibilità dell'esperienza utente effettiva all'inviluppo di qualità, volume e prestazioni della rete degli scenari e alla facilità di implementazione. Le ottimizzazioni consigliate possono essere applicate allo stesso modo per tutti gli endpoint di una determinata categoria.
  
- **Ottimizzazione** degli endpoint sono necessari per la connettività a tutti i servizi di Office 365 e rappresentano oltre il 75% della larghezza di banda, delle connessioni e del volume di dati di Office 365. Tali endpoint rappresentano gli scenari di Office 365 che sono più sensibili alle prestazioni, alla latenza e alla disponibilità della rete. Tutti gli endpoint sono ospitati nei datacenter Microsoft. La percentuale di modifiche apportate agli endpoint in questa categoria dovrebbe essere molto più bassa rispetto a quella degli endpoint nelle altre due categorie. Questa categoria include un piccolo insieme di URL chiave (su ordine di ~ 10) e un set definito di subnet IP dedicate ai carichi di lavoro di core Office 365, ad esempio Exchange Online, SharePoint Online, Skype for business online e Microsoft teams.

    Un elenco condensato di endpoint critici ben definiti dovrebbe essere utile per pianificare e implementare ottimizzazioni di rete di alto valore per queste destinazioni in modo più semplice e veloce.
  ...
  - Ignorare l' *ottimizzazione* degli endpoint sui dispositivi di rete e sui servizi che eseguono l'intercettazione del traffico, la decrittografia SSL, l'ispezione profonda dei pacchetti e il filtro contenuto.
  ...
  - Assegnare la priorità alla riduzione o all'eliminazione del backhauling WAN e facilitare l'uscita diretta basata su Internet distribuita per questi endpoint il più vicino possibile alle posizioni degli utenti/succursali.
...
- **Consenti** gli endpoint sono necessari per la connettività a servizi e funzionalità di Office 365 specifici, ma non sono sensibili alle prestazioni e alla latenza di rete come quelli della categoria *optimize* . L'ingombro complessivo della rete di questi endpoint dal punto di vista della larghezza di banda e del numero di connessioni è anche più piccolo. Questi endpoint sono dedicati a Office 365 e sono ospitati nei Data Center Microsoft. Essi rappresentano un ampio set di micro-servizi di Office 365 e le relative dipendenze (nell'ordine di ~ 100 URL) e dovrebbero variare a una velocità superiore rispetto a quelle della categoria *optimize* . Non tutti gli endpoint di questa categoria sono associati a subnet IP dedicate definite.
  ...
  - Ignorare *Consenti* endpoint sui dispositivi di rete e sui servizi che eseguono l'intercettazione del traffico, la decrittografia SSL, l'ispezione profonda dei pacchetti e il filtro contenuto.
  - Assegnare la priorità alla valutazione di tali endpoint come completamente attendibile dall'infrastruttura di rete e dai sistemi perimetrali.
  - Assegnare la priorità alla riduzione o all'eliminazione del backhauling WAN e facilitare l'uscita diretta basata su Internet distribuita per questi endpoint il più vicino possibile alle posizioni degli utenti/succursali.
  - Verificare che gli indirizzi IP restituiti dalla risoluzione dei nomi DNS corrispondano al percorso di uscita del routing per questi endpoint.
  - Assegnare la priorità a questi endpoint per l'integrazione di SD-WAN per il routing diretto e minimo di latenza al punto di peering Internet più vicino della rete globale di Microsoft.

- Gli endpoint **predefiniti** rappresentano i servizi e le dipendenze di Office 365 che non richiedono alcuna ottimizzazione e possono essere trattati dalle reti dei clienti come normale traffico associato a Internet. Alcuni endpoint di questa categoria potrebbero non essere ospitati nei Data Center Microsoft. Tra gli esempi sono inclusi *https://odc.officeapps.live.com* e *https://appexsin.stb.s-msn.com* .

Per ulteriori informazioni sulle tecniche di ottimizzazione della rete di Office 365, vedere l'articolo [Managing office 365 Endpoints](managing-office-365-endpoints.md).
...
L'obiettivo della sicurezza della rete tradizionale è quello di indurire il perimetro della rete aziendale contro intrusioni e exploit dolosi. Poiché le organizzazioni adottano Microsoft 365, alcuni servizi di rete e dati vengono parzialmente o completamente migrati nel cloud. Per quanto riguarda qualsiasi modifica sostanziale dell'architettura di rete, questo processo richiede una rivalutazione della sicurezza della rete che tiene conto dei fattori emergenti:
...
- **Utilizzare Office 365 Secure Score** Strumento di analisi della sicurezza che consiglia cosa è possibile fare per ridurre ulteriormente i rischi. Secure Score analizza le impostazioni e le attività di Microsoft 365 e le confronta con una linea di base stabilita da Microsoft. Si otterrà un punteggio in base al modo in cui si è allineati con le migliori procedure di sicurezza.
...
- Gestire tutta la sicurezza delle informazioni in maniera olistica, non concentrandosi unicamente sul perimetro
  ...
  - Ridurre i requisiti di sicurezza della rete del server WAN aziendale
...
Per ulteriori informazioni su Microsoft 365 sicurezza e conformità, vedere l'articolo sicurezza di Microsoft [365](https://docs.microsoft.com/microsoft-365/security) e [Microsoft 365 Security](https://docs.microsoft.com/microsoft-365/compliance).
